...
#include <ifaddrs.h>
#include <net/if.h>
#include <signal.h> /* Added for clean shutdown */
...
int main(int argc, char **argv) {
    ...
    LOG("servidor ouvindo na porta %d (Pressione Ctrl+C para encerrar)", port);

    /* Changed infinite loop to respect g_keep_running */
    while (g_keep_running) {
        ...
        if (cli < 0) {
            /* If interrupted by our signal handler, break the loop to shutdown */
            if (errno == EINTR) {
                break;
            }
            continue;
    ...
    LOG("Encerrando servidor. Desmapeando FPGA...");
    fpga_shutdown();
    close(srv);
    LOG("Servidor finalizado com sucesso.");

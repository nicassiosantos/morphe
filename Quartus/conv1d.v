...
module conv1d #(
    ...
    parameter XN_LENGTH      = 1024, // Input signal length
    parameter HN_LENGTH      = 1024, // Filter impulse response length
    parameter YN_LENGTH      = 2047, // Result length (L+M-1)
    parameter XN_ADDR_N_BITS = 10,
    parameter HN_ADDR_N_BITS = 10,
    parameter YN_ADDR_N_BITS = 11
)(
    input  wire                        clk,
    input  wire                        reset_n,
    ...
    localparam [3:0] MULTIPLY   = 4'd5; // [FIX] New state for DSP pipelining
    localparam [3:0] ACCUMULATE = 4'd6; // Multiply-Accumulate (MAC) operation
    localparam [3:0] WRITE_MEM  = 4'd7; // Trigger multi-cycle write to SRAM
    localparam [3:0] WAIT_WRITE = 4'd8; // Wait for write completion
    localparam [3:0] DONE       = 4'd9; // Signal task completion
    ...
    // [FIX] Calculate h[n-k] index using the full width of k to prevent truncation wrap-around
    wire signed [YN_ADDR_N_BITS:0] hn_idx = $signed({1'b0, n}) - $signed({1'b0, k});
    ...
    // [FIX] Pipeline register for multiplication
    reg signed [2*DATA_WIDTH-1:0] mult_result_reg; 

    // [FIX] Shift by 16 bits to maintain the Q16.16 fractional alignment from the registered result
    wire signed [DATA_WIDTH-1:0]  mult_fixed  = mult_result_reg[DATA_WIDTH+16-1:16];
    ...
    always @(posedge clk or negedge reset_n) begin
        if (!reset_n) begin
            state <= IDLE;
            n <= 0; k <= 0; accumulator <= 0; mult_result_reg <= 0;
            ...
            case (state)
                ...
                COMPUTING: begin
                    if (k <= k_end_val) begin
                        // [FIX] Map the calculated indices using the full un-truncated 'k'
                        xn_data_addr <= k; 
                ...
                WAIT_READ: begin
                    // [FIX] Latch data IMMEDIATELY when the valid pulse arrives
                    if (xn_read_done) begin
                        xn_latched    <= $signed(xn_data_out);
                        xn_done_latch <= 1'b1;
                    end
                    if (hn_read_done) begin
                        hn_latched    <= $signed(hn_data_out);
                        hn_done_latch <= 1'b1;
                    end

                    // Transition only when both have been successfully latched
                    if ((xn_read_done || xn_done_latch) && (hn_read_done || hn_done_latch)) begin
                        state <= MULTIPLY; // [FIX] Go to pipelined multiply state
                    end
                end

                MULTIPLY: begin
                    // [FIX] Register the multiplication output to break the combinational path
                    mult_result_reg <= xn_latched * hn_latched;
                    state <= ACCUMULATE;
                end
